...
* @process(in[Int] { 1+ })@ will try to convert "in" message of the exchange to @Int@, increment by 1 and write it back to "in". The is the same as @process(in[Int] { 1+ } .toIn)@ or more verbose @process(in[Int].by { 1+ }.toIn)@
...
* @process(in[Int] collect { case i if i % 2 == 0 => "even" })@
...
* @filter(in[Int] collect { case i if i % 2 == 0 => true })@
